...
ADNI_RAW_CANDIDATES = [
    DATA_ROOT / "ADNI_data_105cases" / "ADNI_data",
    DATA_ROOT / "ADNI_data_105cases",
...
def find_adni_raw_dir():
    ...
    raise FileNotFoundError(
        f"Missing ADNI directory under {DATA_ROOT / 'ADNI_data_105cases'}. "
        "Place shared data in PJ1/data/ADNI_data_105cases/."
    )
...
def rootstrap_install_message():
    return (
        ...
        " export FSLDIR=<FSL-U+5B89-U+88C5-U+8DEF-U+5F84>\n"
...
def fsl_available():
    """Check if FSL is available without raising an error."""
    for cmd in ["fslreorient2std", "flirt", "bet"]:
    ...
    return any(p.exists() for p in mni_candidates)
...
def n4_bias_field_correction_rootstrap(image):
    ...
    corrector.SetMaximumNumberOfIterations([50, 40, 20, 10])
...
def save_sitk(image, path):
    """Save SimpleITK image to disk using nibabel for Unicode path compatibility."""
    path = Path(path)
...
def reorient_to_ras_nibabel(input_path, output_path):
    """Python-only reorientation to RAS (approximates fslreorient2std)."""
    img = nib.load(str(input_path))
    orig_ornt = nib.orientations.io_orientation(img.affine)
    ras_ornt = nib.orientations.axcodes2ornt(("R", "A", "S"))
    if nib.orientations.ornt_transform(orig_ornt, ras_ornt).tolist() == [[0, 1], [1, 1], [2, 1]]:
        nib.save(img, str(output_path))
        return
    img_ornt = img.as_reoriented(ras_ornt)
    nib.save(img_ornt, str(output_path))

def preprocess_case_no_fsl(case_dir, row, label_col, images_dir):
    """Windows-compatible preprocessing without FSL dependency.
    
    Uses nibabel for reorientation, SimpleITK for N4 bias correction.
    Skips FLIRT registration and BET skull stripping.
    """
    case_id = clean_value(case_dir.name)
    output_path = Path(images_dir) / f"{case_id}.nii.gz"
    details = {
        ...
        "input_shape": "",
        "shape_after_reorient": "",
        "shape_after_flirt": "skipped (no FSL)",
        "shape_after_bet": "skipped (no FSL)",
        "shape_after_n4": "",
        "final_shape": "",
        "final_spacing": "",
        "nonzero_ratio": "",
        "normalization_method": "monai_scale_intensity_at_training_time",
        "registration": "nibabel_reorient_ras_only",
        "skull_stripping": "none",
        "n4_method": "simpleitk_n4",
        "min": "",
        "max": "",
        "mean": "",
        "std": "",
    ...
    with tempfile.TemporaryDirectory(prefix=f"rootstrap_{case_id}_") as temp_dir:
        ...
        reoriented = work_dir / f"{case_id}_reorient.nii.gz"
        reorient_to_ras_nibabel(input_nii, reoriented)
        if not reoriented.exists():
            raise RuntimeError("nibabel reorientation finished but output file is missing")
        details["shape_after_reorient"] = shape_text(image_shape(reoriented))

        corrected = n4_bias_field_correction_rootstrap(read_input_as_image(reoriented, "nifti", work_dir, case_id))
        save_sitk(corrected, output_path)
...
def preprocess_case_worker(payload):
    ...
    try:
        use_fsl = payload.get("use_fsl", False)
        if use_fsl:
            ...
            metadata, details = preprocess_case_no_fsl(
                case_dir,
                row,
                label_col,
                Path(payload["images_dir"]),
...
def build_adni_rootstrap_metadata(raw_dir=None, processed_dir=None, require_labels=True, dataset_name="ADNI"):
    ...
    if use_fsl:
        ...
        print("FSL not available - using Python-only preprocessing (nibabel reorient + N4 bias correction)")
